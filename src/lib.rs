...
impl GitAuthenticator {
    ...
    pub fn add_plaintext_credentials(
        mut self,
        ...
        self.plaintext_credentials
            .insert(domain, PlaintextCredentials { username, password });
        self
    }
    ...
    pub fn add_ssh_key_from_file(
        ...
        let password = password.into();
        self.ssh_keys.push(PrivateKeyFileGit {
            private_key,
            public_key,
            password,
        });
...
fn make_credentials_callback<'a>(
    ...
    move |url: &str, username: Option<&str>, allowed: git2::CredentialType| {
        trace!(
            "credentials callback called with url: {url:?}, username: {username:?}, allowed_credentials: {allowed:?}"
        );
        ...
        if allowed.contains(git2::CredentialType::SSH_KEY) {
            if let Some(username) = username {
                if try_ssh_agent {
                    try_ssh_agent = false;
                    debug!(
                        "credentials_callback: trying ssh_key_from_agent with username: {username:?}"
                    );
                ...
                while let Some(key) = ssh_keys.next() {
                    ...
                    if let Err(e) = temp_file.write_all(pem_key.as_bytes()) {
                        debug!("Failed to write PEM key to temp file: {}", e);
                        continue;
                    }
                    ...
                    let prompter = Some(prompter.as_prompter_mut())
                        .filter(|_| authenticator.prompt_ssh_key_password);
                    match new_key.to_credentials(username, prompter, git_config) {
                        Ok(x) => return Ok(x),
                        Err(e) => {
                            debug!(
                                "credentials_callback: failed to use SSH key from file {:?}: {e}",
        ...
        if allowed.contains(git2::CredentialType::USER_PASS_PLAINTEXT) {
            ...
            if let Some(credentials) = authenticator.get_plaintext_credentials(url) {
                debug!(
                    "credentials_callback: trying plain text credentials with username: {:?}",
...
#[derive(Debug, Clone)]
struct PrivateKeyFileGit {
    private_key: PathBuf,
    public_key: Option<PathBuf>,
    password: Option<String>,
}

impl PrivateKeyFileGit {
    fn to_credentials(
        &self,
        username: &str,
